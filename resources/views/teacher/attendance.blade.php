...
@section('content')
    ...
    <style>th,td{border: solid 1px black;} select.rm-arrow{-moz-appearance: none;-webkit-appearance: none; width: 30px; text-align: center;}</style>
    <table>
        <tr>
            <th>Uczeń</th>
        ...
        <tr>
            ...
            @foreach($all_lessons as $column)
                <td>
                    ...
                    @if($col_number>1)<button title="Kopiuj wpisy frekwencji z poprzednich zajęć" onclick="copy_from_previous({{$col_number}})">cp</button>@endif
                    <button title="Uzupełnij puste pola obecnościami" onclick="empty_to_plus({{$col_number}})">+</button>
                    @endif
                </td>
            <?php $col_number++; ?>
            @endforeach
    ...
    <div>
        To może być po prawej stronie tabelki
        <p>Legenda:</p>
        <ul>
            @foreach($statuses as $status)
