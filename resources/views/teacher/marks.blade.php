...
<h3>
    <form>
        ...
        <?php
            ...
            ->get(); ?>
        <select id="lesson" onchange="redirect(this)">
...
<button onclick="show_cat_form()">Dodaj kategorię ocen</button>
<?php
...
<table>
    ...
    <tr>
        ...
        @endforeach
        @if($i==1)
        <td rowspan="{{count($students)}}">Chciałbym, żeby ta kolumna, była tak rozciągnięta, żeby tableka zajmowała stałą szerokość na ekranie.</td>
        @endif
        <?php
...
<button onclick="save_marks()">Zapisz oceny</button>


<div id="add-category-form" style="position: absolute; top: 50%; left:40%; border: 1px black solid; width: 20%; background-color: white; display: none;">
    Dodaj kategorię:
    <form method="post" action="{{ url('addMarkCategory') }}" >
        ...
        Waga: <input type="number" name="weight" value="1"><br>
        Licz do średniej: <input type="checkbox" name="average" checked><br>

        <button>Dodaj</button>
        <button type="reset" onclick="hide_cat_form()">Anuluj</button>
    </form>
</div>
